...
In this post I'll briefly explain what git hooks are, introduce `hk`, and show some practical examples of using with with Ruby and Rails projects. 

## Git hooks

Git hooks are shell scripts which run automatically in response to particular events. These are stored in a hidden `.git/hooks` directory, either per project, or globally.

The most commonly used kind of Git hook is a `pre-commit` hook, typically used for linting and formatting. 

While you can write git hooks by hand, people typically make use of hook manager tools. These usually provide higher-level abstractions, or make it easier to share hooks within a team, and keep them up to date.
...
Until recently, my experience has been that people would configure their editor to show linting, and then configure format-on-save, for example with Ruby LSP.

Now that more and more code is being written by agents, there is often no editor tooling involved. You could configure your agent to be aware of your linting tools, but this is not ideal:

* It's slow. The latency as the agent goes back and forth with the linting tools is a drag.
...
[`hk`](https://hk.jdx.dev) is a project by Jeff Dickey (`jdx`) who is also the author of [`mise`](https://mise.jdx.dev), a popular tool for managing development environments.
...
- Performance: It's written in Rust and runs tools concurrently.
...
`hk` has good documentation on how to get started, so I won't repeat the general steps here. Instead I'll focus on Ruby and Rails.
...
We'll start with RuboCop, since it's so common in the Ruby world.

Assuming that your project uses RuboCop, let's start by adding a step for that.
...
```
demo.rb:1:1: C: [Correctable] Style/FrozenStringLiteralComment: Missing frozen string literal comment.
puts "hello world"
^
demo.rb:1:6: C: [Correctable] Style/StringLiterals: Prefer single-quoted strings when you don't need string interpolation or special symbols.
puts "hello world"
     ^^^^^^^^^^^^^
```
...
You notice that one offense is fixed, but not the other. This is because `Style/FrozenStringLiteralComment` is considered an unsafe autocorrection in RuboCop.

Since the file is invalid, the commit is rejected. We need to manually fix the other offense by adding the `# frozen_string_literal: true` line, and then commit again.

If you were using a coding agent, it can handle this fix you. And depending on the model's capabilities, it may be able to check is `# frozen_string_literal: true` is actually safe or not.

## Adding Herb

Let's now look at how we could add support for a tool which isn't currently natively supported by `hk`.

`hk` has a builtin for `erb` linting, but the modern tool is a linter and formatter for ERB files: `herb`. Its focus on performance makes it a great match for `hk`.
...
And as `hk` doesn't have a builtin for `herb`, we'll need to add some custom configuration:
...
Along with `mise`, `hk` has become one the tools that I rely on every day when coding. Whether your writing code by hand, or using an agent, it will catch linting and formatting issues quickly and effectively, allowing you to focus on the more interseting things.
