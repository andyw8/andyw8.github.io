...
Although git hooks have been around for a long time, I have found that they are not so commonly used. Instead, people would rely on their editor for linting, and make use of format-on-save, for example with [Ruby LSP](https://github.com/shopify/ruby-lsp).

Now that more and more code is being written by agents, there is often no editor tooling involved so that approach . You could use a promopt (e.g. in `CLAUDE.md`) to make your agent to be aware of your linting tools, but this is not ideal:
